...
/**
 * Assigning userName and userId to global variables for further use.
 */
var userName;
...
/**
 * This function takes one postId parameter, reads all the comments from that post in firebase (comments array field)
 * and calls populateComment for each commentId.
 * @param postId the post id in firestore database
 */
...
/**
 * This function is responsible for reading the comment with the commentId given and populating it
 *  into a certain post in home.html.
 * @param commentId represents the commentId to be read from firebase and populated
 * @param postId is used as an element id in home.html so it's used in a querySelector to populate the
 * comment in the right post.
 */
...
/**
 * This function is called when the Add Comment button is pressed by the user and it's responsible for
 * populating the comment into the page and writing it to our firestore database.
 * @param postId the html id  that represents the post on where to populate the new comment
 * @returns 1 and exits the function if comment is empty
 */
